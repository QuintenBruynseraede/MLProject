...
flags.DEFINE_string("modeldir", "policies", "Directory where policies will be saved.")
...
def main(_):
  ...
  with tf.Session() as sess:
    ...
    expl_policies_avg = AgentPolicies(env,players)
    ...
    for pid, agent in enumerate(agents):
        policy_to_csv(env.game, expl_policies_avg, f"{FLAGS.modeldir}/test_p_"+now.strftime("%m-%d-%Y_%H-%M")+"_{pid+1}.csv")
